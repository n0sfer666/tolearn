...
и с `\r\n`: эхо там даёт `findstr`, а он живёт в кодовой странице консоли и строку без
терминатора не считает строкой. Байтовая верность кириллицы через stdin проверяется на
unix — на пути продукта cmd не стоит, харнесс запускается напрямую.
